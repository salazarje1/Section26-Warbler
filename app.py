...
def add_like(msg_id):
    ...
    likes = Likes.get_likes(g.user.id)
    user_id = g.user.id
    message_id = msg_id
...
def homepage():
    ...
    if g.user:
        ids = User.get_user_id(g.user.id, g.user.following)
        messages = Message.get_messages(ids)
        likes = Likes.get_likes(g.user.id)

        return render_template('home.html', messages=messages, ids=ids, likes=likes)
